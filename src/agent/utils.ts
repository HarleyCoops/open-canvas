...
export const formatReflections = (
  ...
  // Create safe reflection object first
  const safeReflections = {
    styleRules: Array.isArray(reflections?.styleRules)
      ? reflections.styleRules
      : [],
    content: Array.isArray(reflections?.content) ? reflections.content : [],
  };

  const styleString = `The following is a list of style guidelines previously generated by you:
<style-guidelines>
- ${
    safeReflections.styleRules.length
      ? safeReflections.styleRules.join("\n- ")
...
<user-facts>
- ${
    safeReflections.content.length
      ? safeReflections.content.join("\n- ")
      : "No memories/facts found."
  }
</user-facts>`;

  if (extra?.onlyStyle) return styleString;
  if (extra?.onlyContent) return contentString;
  return styleString + "\n\n" + contentString;
};
